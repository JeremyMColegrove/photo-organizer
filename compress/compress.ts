...
export async function compressImagesInFolder(
	files: FileList,
	maxSizeKB = 500,
	opts: { quality?: number; resizeMaxDimension?: number } = {},
): Promise<void> {
	...
	const b = bar.start(0, files.length, {
		task: "Compressing",
	});
	let totalOriginalBytes = 0;
	let totalCompressedBytes = 0;
	let processedCount = 0;
	const qualityDefault = typeof opts.quality === "number" ? opts.quality : 80;
	const maxDim =
		typeof opts.resizeMaxDimension === "number"
			? opts.resizeMaxDimension
			: 4000;
	for (const file of files) {
		b.increment(0, { detail: path.basename(file) });
		const inputPath = file; //path.join(folderPath, img.name);
		const outputPath = file; //path.join(folderPath, file.name); // overwrite same file
		...
		const quality = qualityDefault; // start quality
		const compressed = await sharp(buffer)
			...
			.resize({
				...
				height:
					metadata.height && metadata.height > maxDim ? maxDim : undefined,
				fit: "inside",
